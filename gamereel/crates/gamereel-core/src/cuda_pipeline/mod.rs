...
impl CudaConverter {
    ...
    /// rest of the process).
    pub fn new(width: u32, height: u32) -> Result<Self, GamereelError> {
        ...
        let ctx = CudaContext::new(0).map_err(|e| {
            GamereelError::CustomError(format!("CudaContext::new(0) failed: {e:?}"))
        })?;
        let stream = ctx.default_stream();

        let ptx = cudarc::nvrtc::compile_ptx(kernels::RGBA_TO_NV12).map_err(|e| {
            GamereelError::CustomError(format!("nvrtc compile of rgba_to_nv12 failed: {e:?}"))
        })?;
        let module = ctx.load_module(ptx).map_err(|e| {
            GamereelError::CustomError(format!("CUDA module load failed: {e:?}"))
        })?;
        let kernel = module.load_function("rgba_to_nv12").map_err(|e| {
            GamereelError::CustomError(format!("load rgba_to_nv12: {e:?}"))
        })?;
    ...
    pub fn convert(&mut self, rgba_host: &[u8]) -> Result<(), GamereelError> {
    ...
    pub fn synchronize(&self) -> Result<(), GamereelError> {
    ...
    pub fn read_y_to_host(&self) -> Result<Vec<u8>, GamereelError> {
        self.stream.clone_dtoh(&self.y_dev).map_err(map_err)
    }

    /// Diagnostic companion to [`read_y_to_host`].
    pub fn read_uv_to_host(&self) -> Result<Vec<u8>, GamereelError> {
...
fn map_err<E: std::fmt::Debug>(e: E) -> GamereelError {
    GamereelError::CustomError(format!("cuda: {e:?}"))
}
