...
| - [ ] | 3m | `#D1c` | **(新)** Material 抽屉 `Generate all` 三 lane → 一次扇出整个 palette，每 slot 一个 toast，busy guard 挡重复点 |
| - [ ] | 3m | `#D1d` | **(新)** View `Flat` ↔ `Textured` 真切换：已生纹理的 slot 立体显示 PNG，未生纹理的 slot fallback 平色 |
| - [ ] | 2m | `#cloud-chip` | **(新)** v0.11.A 云模式 chip：启动 1.5s 探测 hfrog，在线显示 `Cloud: online`，失败显示 `Local mode`，点击重试 |
| - [ ] | 5m | `#queue-gate` | **(新/高优)** Rustyme vs Celery 复测关口：看 `phase0-2-initial.md` 只能当初始信号，最终结论必须等 Lua/payload/group/recovery/CPU-time 全部补齐 |
...
**①  小计 ≈ 106 min**（`#BLOCK-cli` / `#BLOCK-gui` / `#COMPOSER-mock` / `#COMPOSER-publish` / `#D1c` / `#D1d` / `#shortcuts` 各 3 min；`#cloud-chip` 2 min；`#D1-material` / `#D1-slotgen` 各 4 min；`#queue-gate` 5 min）。建议顺序就按上表从短到长跑，连续打钩很有成就感。
...
- **#D1c**（3 min）Material 抽屉 `Generate all` 三 lane 一键扇出
  1. 准备 palette：至少 3 个活色（默认就有，或者按 `+` 加几个）
  2. 展开左栏 **▾ Material** → 最下面应该看到 `Generate all (N live slots):` 和三个按钮 `[Mock] [CPU] [Fal]`
  3. 点 **`Mock`** → toast 连续刷出 `Generating slot #0: ...` / `#1` / `#2` ... → 紧接着 `Mock done · slot #0 · ...` / ... 共 N 次
  4. `~/.cache/maquette/textures/` 应该多了 N 个 `<sha>.png`（每 slot 不同 prompt → 不同 sha）
  5. 立刻再点 **Mock** → 这次 toast 都是 `slot #N already generating — waiting...`（busy guard 全挡掉）—— 等前一波完成再点才会真发
  6. palette 删到 0 个活色 → 三个按钮变 disabled，hover 提示 "No live palette slots — add a colour first"
  7. 配好 env 后 `CPU` / `Fal` 同样能扇出 N 个真生成
  - **不通过现象**：按钮可点但没有 toast 说明 message 没接上 / busy guard 不工作让重复发出 N 个 task → 完整 stderr 发我

- **#D1d**（3 min）View `Flat` ↔ `Textured` 真切换
  - **前置**：先 `#D1c` 给至少一个 slot 出过纹理（Mock 即可）
  1. Material 抽屉 → `View:` 点 **`Textured`**
  2. 3D 预览里那个有纹理的 slot 对应的 cube 应该立刻**贴上 PNG**（toon 单调高光 + bands 还在，但底色变成了纹理）
  3. 没生过纹理的 slot 仍然是平色（不是 magenta 缺纹理玫红）—— 这是 fallback 行为
  4. 切回 **`Flat`** → 所有 cube 变回平色 toon
  5. 在 Textured 模式下再生一个新 slot 的纹理（右键或 `G`）→ 不用切换 mode，下一帧 cube 应该自动换上新纹理（palette 变更触发 mesh 重建）
  6. 边画边切：画几笔 cube → 切 Textured → 再画几笔 → cube 实时贴图正常工作
  - **不通过现象**：`Textured` 切了但 cube 不变 / 切到 Textured 后 cube 是 magenta 缺纹理色 / 切回 Flat 后还残留纹理 → 完整 stderr `RUST_LOG=info maquette` 看 `texture_registry:` 行有没有 warn 报 "not on disk" 之类
  - **诊断**：`ls ~/.cache/maquette/textures/` 看 PNG 是不是真在；project 里 `palette[N].texture.cache_key` 是不是和该 sha 对得上（用 `cargo run --bin maquette-cli -- validate <file.maq>` 是不是绿）

- **#cloud-chip**（2 min）v0.11.A 云模式状态 chip
  1. 正常网络下启动 GUI，底部状态栏右侧 `Maquette · v0.8-dev` 左边应先短暂出现 `Cloud: checking...`，1-2 秒内变成 **`Cloud: online`**
  2. hover 这个 chip：tooltip 应显示 hfrog base URL（默认 `https://hfrog.gamesci-lite.com`）、last checked、"Click to probe again"
  3. 点击 chip：应重新进入 `Cloud: checking...`，随后回到 `Cloud: online`
  4. 用坏 URL 验证本地降级：
     ```sh
     MAQUETTE_HFROG_BASE_URL=http://127.0.0.1:1 cargo run --bin maquette
     ```
     1.5 秒左右应显示 **`Local mode`**，tooltip 里有失败原因；File → Save / Open 仍正常走本地
  5. 从坏 URL 状态点击 chip：仍可手动重试，不会卡 UI
  - **不通过现象**：启动被 hfrog 探测卡住超过 2 秒 / 离线时 Save/Open 不可用 / chip 不显示 `Local mode` / 点击不重试。完整 stderr 发我。

- **#queue-gate**（5 min）Rustyme vs Celery 复测关口复盘
  - **目标**：防止我们只看 no-op smoke 数据就继续押 Rustyme。这个 gate 现在是 roadmap 最高优插入版本 `v0.10-QA`，在 D-2 / E / v0.11.B 之前。
  1. 打开 `maquette/docs/handoff/rustyme-vs-celery-plan.md`
  2. 确认顶部 **Non-Negotiable Decision Guard** 里 10 个必测类都在：EchoHook、Lua hook、Celery prefork、Celery threads、payload、group/chord、failure recovery、Maquette workload、CPU-time/RSS、**long-request fan-out overhead**
  3. 打开 `maquette/reports/rustyme-vs-celery-2026-05/summaries/phase0-2-initial.md`
  4. 确认 **Conclusion Guard** 明确写着：当前报告不是最终 recommendation，只回答 no-op EchoHook vs Celery prefork 的 runtime overhead
  5. 确认 `NEXT.md` 里 `v0.10-QA` 在 roadmap snapshot 中，且 "Now — start here next session" 第一项是 finish Rustyme vs Celery gate，而不是 v0.11.B / D-2 / E
  6. 如果你看到任何地方把 `Rustyme EchoHook 5k` 的结果写成“最终胜出”，直接标失败；那是误导
  - **通过标准**：roadmap 清楚表达“队列后端选型是阻塞门”，报告清楚表达“当前数据只是初始信号”
  - **不通过现象**：缺 Celery threads / Lua parity / payload / failure recovery / CPU-time / long-request fan-out 任一项，或者 NEXT 还让队列依赖功能排在 gate 前面
