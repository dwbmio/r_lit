...
**①  小计 ≈ 82 min**（`#BLOCK-cli` / `#BLOCK-gui` / `#COMPOSER-mock` / `#COMPOSER-publish` 各 3 min）。建议顺序就按上表从短到长跑，连续打钩很有成就感。
...
- [x] `#TEX-B-fal` — Maquette 侧无活；剩下的 `FAL_KEY` 配置是 sonargrid 那边的 ops 工作。
