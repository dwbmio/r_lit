...
| **M3** | CUDA hwframes + h264_nvenc (cudarc kernel) | **456 (1.23× M2)** | (CUDA-only path, e2e measurement) | unchanged from M2 (encoder identical) | — | Full GPU pipeline: cudarc RGBA→NV12 kernel + ffmpeg CUDA hwframes pool; eliminated the sws_scale CPU slug. **0 MB VRAM leak across 100 cycles**. perf_main multiplier muted because the scene is too small to amortize compositing cost — real wins land in M4 (wgpu compositor outputs straight to GPU). See [O-012..014](docs/optimization-log.md#m3--full-gpu-pipeline-cudarc--ffmpeg-cuda-hwframes). |
