...
Release detect 矩阵覆盖：`bulk_upload` `img_resize` `textexture` `mj_atlas` `group_vibe_workbench` `maquette`。GUI 项目（`maquette` `group_vibe_workbench`）在 macOS 走 codesign + notarize 出 .dmg。
