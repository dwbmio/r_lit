...
- [x] **#TEX-B — Rustyme 端到端联调** ✅ 2026-04-27 晚已贯通
      cpu solid + cpu smart LLM + fal routing + revoke + 缓存 + 顺手修了一个潜伏的死循环 bug。
      6 项验证全过，详见 `v0.10b-bis-complete.md` § 4 / NEXT.md。**这一项已结，无需手验。**

- [~] **#TEX-C — schema v4 lib 部分** ✅ 2026-04-27 晚已 ship（`v0.10c1-complete.md`）
      v3 → v4 加载默认值、v4 round-trip、palette_meta 长度自动修复、`legacy write_project` 也产 v4 文件 —— 25 个新单测全过。
      **`override_hint` 编辑进 undo 链** 这一条留给 D-1（GUI 加文本框 + EditHistory 扩展时一并做）。
      lib 端今天就能用 CLI 验：`maquette-cli info v3.maq` / `palette export v3.maq …` 都能开。
      GUI 端的"open v3 → 改一笔 → save → 看到 version=4 + palette_meta"留到 GUI 加 Generate 按钮时一起跑。

刚刚一波代码（v0.9 polish + v0.10 A/B + v0.10 B-bis）落盘。下面这些**全是新到手**、还没你亲眼验证过的：
...
| ~~`#TEX-B`~~ | ~~Rustyme 端到端~~ ✅ **完成** — 见上面置顶勾选项 | 0 min |

总预计 ~30–40 min 能把上面非阻塞项过一遍。`#TEX-B` 不再阻塞 — sonargrid 那边其实早把 Stage 1 + Stage 2 都做完了（甚至多做了一条 LLM 智能解析的 cpu 队列），今晚已经联调贯通。
...
- **#TEX-C**（v0.10 C-1 lib 部分已 ship 2026-04-27 晚 · GUI 部分留 D-1）schema v4 向后兼容验证
  ...
  - lib 端可手验：用 v0.9 版本存的 `.maq` 文件，跑 `maquette-cli info <file>.maq` / `maquette-cli palette export <file>.maq …` 应都能正常打开（无 GUI 即可验）。
