...
use crate::{stage::scene::Scene, GamereelResult, RuntimeCtx};
use ffmpeg_next::{self as ffmpeg};
use image::GenericImageView as _;

use encoder_pick::{pick_h264_encoder, EncoderChoice, EncoderPreference};

pub fn init_env() -> crate::GamereelResult<()> {
    ffmpeg_next::init()?;
    Ok(())
}
...
mod cuda_path {
    ...
    pub(super) fn create_scene_stream(
        ...
        let codec = ffmpeg::codec::encoder::find_by_name("h264_nvenc")
            .ok_or_else(|| crate::error::GamereelError::CustomError(
                "h264_nvenc not available — cannot use CUDA pipeline".into(),
            ))?;
        log::info!("gamereel-core: CUDA pipeline → h264_nvenc");
        ...
        for frame_number in 0..total_frames {
            ...
            unsafe {
                ...
                if send_rc < 0 {
                    ...
                    return Err(crate::error::GamereelError::CustomError(format!(
        ...
        octx.write_trailer()?;
        log::info!(
            "gamereel-core: wrote {} ({} frames @ {}x{} via CUDA hwframes + h264_nvenc)",
...
fn create_scene_stream_inner(
    ...
    let codec = ffmpeg::codec::encoder::find_by_name(choice.codec_name).ok_or_else(|| {
        crate::error::GamereelError::CustomError(format!(
            "encoder '{}' was selected but find_by_name returned None",
            choice.codec_name
        ))
    })?;
    log::info!(
        "gamereel-core: selected encoder '{}' ({})",
        choice.codec_name,
    ...
    let stream_time_base = octx
        .stream(0)
        .ok_or_else(|| {
            crate::error::GamereelError::CustomError("output stream 0 missing after write_header".into())
        })?
        .time_base();
    ...
    octx.write_trailer()?;
    log::info!(
        "gamereel-core: wrote {} ({} frames @ {}x{} via {})",
        output.display(),
        total_frames,
        width,
        height,
