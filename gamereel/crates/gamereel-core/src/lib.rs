...
type GamereelResult<T> = Result<T, error::GamereelError>;
...
impl RuntimeCtx {
    ...
    pub fn init(&mut self, proj_path: Option<PathBuf>) -> crate::GamereelResult<()> {
        if !self.init {
            ffmpeg_inc::init_env()?;
            self.init = true;
        }
    ...
    pub fn load_loc_image<'a>(&mut self, rel_path: &str, id: &str) -> GamereelResult<String> {
        let t_pf = self
            .source_path
            .clone()
            .expect("Ensure111")
    ...
    pub fn set_textures_cache(&mut self, img: &DynamicImage, name: &str) -> GamereelResult<String> {
