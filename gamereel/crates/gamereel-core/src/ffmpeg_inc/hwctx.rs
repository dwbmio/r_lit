...
impl CudaHwContext {
    ...
    pub fn new(width: u32, height: u32, pool_size: i32) -> Result<Self, GamereelError> {
        ...
        unsafe {
            ...
            if frames_ref.is_null() {
                ...
                return Err(GamereelError::CustomError(
            ...
            log::info!(
                "gamereel-core: CudaHwContext ready ({width}x{height}, NV12 pool size={pool_size})"
            );
    ...
    pub unsafe fn allocate_frame(&self) -> Result<*mut ffsys::AVFrame, GamereelError> {
        let frame = ffsys::av_frame_alloc();
        if frame.is_null() {
            return Err(GamereelError::CustomError("av_frame_alloc returned null".into()));
        }
...
fn av_err(label: &str, rc: c_int) -> GamereelError {
    let mut buf = [0i8; 256];
    unsafe {
        ...
        let msg = std::ffi::CStr::from_ptr(buf.as_ptr() as *const _)
            .to_string_lossy()
            .into_owned();
        GamereelError::CustomError(format!("{label} failed (rc={rc}): {msg}"))
    }
}
