...
Release detect 矩阵覆盖：`bulk_upload` `img_resize` `textexture` `mj_atlas` `group_vibe_workbench` `maquette` `deskpet`。GUI/桌面项目（`maquette` `group_vibe_workbench` `deskpet`）在 macOS 走 codesign + notarize 出 .dmg。

此外，`ci-all-in-one/task/ci/pipeline/r_lit/Jenkinsfile.binary-build` 提供按工具手动触发的 Jenkins 流水线（dev.gamesci-lite.com），其 `toolMap` 需与本仓库工具同步登记；GUI 工具（如 `deskpet`）需在该 map 内声明 Linux 构建所需系统依赖（`system_deps`），Windows MinGW 交叉编译 bevy 不可靠故默认跳过。
