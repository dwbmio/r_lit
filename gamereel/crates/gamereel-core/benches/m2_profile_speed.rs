//! M2 criterion bench: encoder throughput per [`EncoderProfile`].
//!
//! Mirrors `m1_quickwin.rs` but iterates over the four EncoderProfile
//! variants instead of raw encoder names. Same 720x1080 / 10 s synthetic
//! source. Long iterations (300 frames) so NVENC's ~300–500 ms session
//! cold-start amortizes — without that, the measurement is dominated by
//! startup, not steady-state encoding.
//!
//! Acceptance reference (vs M1 = 619 fps shootout winner / 377 fps e2e):
//!   * Fast:      ≥ 1.10 × Balanced fps
//!   * Balanced:  reproduces M1 numbers within ±15 %
//!   * TikTokHQ:  ≥ 0.50 × Balanced fps (slow but high-quality preset
//!                p6 + AQ + lookahead is intentionally heavier)
//!   * IgReelsHDR: same as TikTokHQ in M2 (HDR adds in M3)
...
const H: u32 = 1080;
const FPS: i32 = 30;
const FRAMES_PER_ITER: u32 = 300; // 10 s
...
fn encode_for_profile(out_path: &PathBuf, profile: EncoderProfile, frames: u32) -> u64 {
    ...
    let codec_h264 = codec::encoder::find_by_name(choice.codec_name)
        .unwrap_or_else(|| panic!("encoder '{}' not present", choice.codec_name));
    ...
    // Hoisted scaler + frames (M1 layout).
...
fn bench_profiles(c: &mut Criterion) {
    ...
    let mut group = c.benchmark_group("encode_profile_720x1080_10s");
    ...
    for prof in [
        ...
        EncoderProfile::TikTokHQ,
        EncoderProfile::IgReelsHDR,
    ] {
        let label = format!("{prof:?}");
        group.bench_with_input(BenchmarkId::from_parameter(&label), &prof, |b, &p| {
            b.iter(|| {
                idx += 1;
                let path = dir.path().join(format!("p_{idx}.mp4"));
                let bytes = encode_for_profile(&path, p, FRAMES_PER_ITER);
                assert!(bytes > 1024);
