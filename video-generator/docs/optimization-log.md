...
**Measurement**
- Cold workspace build went from 6m 53s (movie-maker only) to 3m 26s (movie-maker + demo) on the same machine. Most savings came from `ffmpeg-next` git-source caching in the workspace target dir, not the deletions.
...
- Followup: M5's batch-100 bench will exercise this with real movie-maker output (much closer to TikTok content); revisit Fast profile selection then.
...
- Followup: when M5 lands, re-run the grid against real movie-maker output (game data cards) to revisit AQ for Balanced.
